...
def get_threshold(target):
    if target < 5:
        return 20
    if target >=5 and target < 10:
        return 30
    if target >=10 and target < 20:
        return 40
    if target >=20 and target <= 30:
        return 45
...
class Sparse(nn.Module):
    ...
    def get_k(self, epoch,l0_norm):
        l0_norm_threshold = get_threshold(self.target_size)
        start = 250
        maximum = 1
...
class ZhangNet(nn.Module):
    def __init__(self, bands, number_of_classes, last_layer_input, dataset, target_size):
        super().__init__()
        self.dataset = dataset
        self.bands = bands
        ...
        self.sparse = Sparse(self.dataset, self.target_size)
...
class Algorithm_v3(Algorithm):
    def __init__(self, target_size:int, dataset, tag, reporter, verbose, test):
        ...
        self.zhangnet = ZhangNet(self.dataset.get_train_x().shape[1], self.class_size, self.last_layer_input,
                                 self.dataset.get_name(), self.target_size).to(self.device)
    ...
    def get_cache_tag(self):
        return get_threshold(self.target_size)