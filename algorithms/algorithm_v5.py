...
class Algorithm_v5(Algorithm):
    ...
    def get_selected_indices(self):
        ...
        self.X_val.requires_grad = True
        self.y_val = self.y_val.type(torch.LongTensor).to(self.device)
        model = ModelWrapper(self.zhangnet.to(self.device))
        baseline_data = torch.zeros_like(self.X_val).to(self.device)
        baseline_data.requires_grad = True
        ig = IntegratedGradients(model)
        attributions = ig.attribute(self.X_val, baselines=baseline_data, target=self.y_val)
        shap_values = torch.mean(torch.abs(attributions), dim=0)
        shap_band_indx = torch.argsort(shap_values, descending=True).tolist()
        print(shap_band_indx)
        print(self.all_indices)
        self.set_all_indices(shap_band_indx)
        self.set_selected_indices(shap_band_indx[:self.target_size])
        self.set_weights(shap_values)
