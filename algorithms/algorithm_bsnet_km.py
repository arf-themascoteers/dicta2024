...
class BSNetFC(nn.Module):
    def __init__(self, bands):
        super().__init__()
        self.bands = bands
        self.weighter = KAN(width=[self.bands, self.bands], grid=5, k=3, seed=42, device='cuda')
        self.encoder = KAN(width=[self.bands,20,self.bands], grid=5, k=3, seed=43, device='cuda')
...
class Algorithm_bsnet_km(Algorithm):
