...
class Sparse(nn.Module):
    ...
    def forward(self, X, epoch, target_size):
        X = torch.where(torch.abs(X) < self.get_k(epoch, target_size), 0, X)
        return X

    def get_k(self, epoch, target_size):
        if target_size > 15:
            return 0
        k = 0.5 - (0.2 / 10) * (target_size - 5)
        if epoch < 50:
            return 0.0
        elif epoch > 500:
            return k
        else:
            return (epoch - 50) * (k / (500 - 50))
...
class ZhangNet(nn.Module):
    ...
    def forward(self, X, epoch, target_size):
        ...
        sparse_weights = self.sparse(channel_weights, epoch, target_size)
    ...
    def get_selected_indices(self):
        ...
        for epoch in range(self.total_epoch):
            ...
            for batch_idx, (X, y) in enumerate(dataloader):
                optimizer.zero_grad()
                channel_weights, sparse_weights, y_hat = self.zhangnet(X, epoch, self.target_size)
